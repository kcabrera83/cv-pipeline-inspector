...
def test_models():
    results = {}
    results['defect'] = evaluate_model(os.path.join(MODEL_DIR, 'defect_classifier.pkl'), X, y)
    results['severity_estimator'] = evaluate_model(os.path.join(MODEL_DIR, 'severity_estimator.pkl'), X, y)
    return results
